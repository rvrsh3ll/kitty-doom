# Contributing to `kitty-doom`

:+1::tada: First off, thanks for taking the time to contribute! :tada::+1:

The following is a set of guidelines for contributing to [kitty-doom](https://github.com/jserv/kitty-doom)
...
Visit our [Issues page on GitHub](https://github.com/jserv/kitty-doom/issues) to search and submit.
...
* Code must strictly adhere to the established C coding style (refer to the guidelines below).
  ...
  **Run `make indent` before submitting** to automatically format all code.
* Shell scripts must be formatted before submission using `make indent` (honors `.editorconfig` settings).
* Python scripts must be formatted before submission using `make indent`.
...
**clang-format version 21** (major version must match CI)
- macOS: `brew install llvm@21`
- Linux: Install from [apt.llvm.org](https://apt.llvm.org/)
- Documentation: [ClangFormat](https://clang.llvm.org/docs/ClangFormat.html)

**shfmt** (latest version, automatically honors `.editorconfig`)
- macOS: `brew install shfmt`
- Linux: Download from [releases](https://github.com/mvdan/sh/releases)

### Formatting Workflow

Before submitting code:
1. **Format automatically**: Run `make indent` to format all C/C++ and shell scripts
2. **Verify formatting**: Run `.ci/check-format.sh` to check against CI requirements
   - Shows exact differences if formatting issues are detected
   - Must pass before pull request approval

## Coding Style for Shell Script

Shell scripts must be clean, consistent, and portable. The following `shfmt` rules (defined in `.editorconfig`) are enforced project-wide:
